...
#endif
{
    ...
    if (isRunning && widgetPara.bannerEnable && videoWidth > 0 && videoHeight > 0) {
        ...
        //悬浮条如果宽度不够则不显示文字信息
        if (bannerWidget->width() > 200) {
            if (!onlyAudio && videoWidth > 0) {
                bannerWidget->showInfo(QString("分辨率: %1 x %2").arg(videoWidth).arg(videoHeight));
            }
        } else {
