...
{
    onvifBase->getServices();
    onvifBase->getProfiles();
    return onvifBase->getStreamUri(this->profile);
}
...
bool OnvifDevice::ptzFocus(const QString &videoSource, int speed)
{
    return onvifPtz->ptzFocus(videoSource, speed);
}
