...
//https://blog.csdn.net/weixin_39510813/article/details/119613386
bool OnvifPtz::ptzFocus(const QString &videoSource, int speed)
{
    ...
    //读取文件传入带用户认证的通用头部数据和其他参数构建要发送的数据
    QString file = OnvifHelper::getFile(":/onvifsend/PtzFocus.xml");
    QString head = device->getHeadData();
    file = file.arg(head).arg(videoSource).arg(speed);
    ...
    //拿到请求结果并处理数据
    QByteArray dataReceive;
    return device->checkData(reply, dataReceive, "动态调焦");
}
